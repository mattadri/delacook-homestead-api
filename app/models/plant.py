...
class PlantNotes(db.Model):
    __tablename__ = 'plantnotes'

    id = db.Column(db.Integer, primary_key=True)
    created = db.Column(db.DateTime, default=db.func.current_timestamp())
    modified = db.Column(db.DateTime, default=db.func.current_timestamp(), onupdate=db.func.current_timestamp())
    plant_cloning_fk = db.Column(db.Integer, db.ForeignKey('plants.id'))
    plant_cloning = db.relationship('Plant', backref=db.backref('plantnotes'))
...
class PlantLineage(db.Model):
    ...
    previous_plant_generation_fk = db.Column(db.Integer, db.ForeignKey('plantgenerations.id'))
    previous_plant_generation = db.relationship('PlantGeneration', backref=db.backref('plantlineages'))
...
class PlantGeneration(db.Model):
    ...
    created = db.Column(db.DateTime, default=db.func.current_timestamp())
    modified = db.Column(db.DateTime, default=db.func.current_timestamp(), onupdate=db.func.current_timestamp())
    date_seeds_collected = db.Column(db.DateTime)
...
class PlantClone(db.Model):
    ...
    planting_fk = db.Column(db.Integer, db.ForeignKey('plantings.id'))
    planting = db.relationship('Planting', backref=db.backref('plantclones'))
...
class PlantCloning(db.Model):
    ...
    created = db.Column(db.DateTime, default=db.func.current_timestamp())
    modified = db.Column(db.DateTime, default=db.func.current_timestamp(), onupdate=db.func.current_timestamp())
    plant_clone_fk = db.Column(db.Integer, db.ForeignKey('plantclones.id'))
    plant_clone = db.relationship('PlantClone', backref=db.backref('plantclonings'))
    date_cloned = db.Column(db.DateTime)
    amount_cloned = db.Column(db.Integer)
    amount_rooted = db.Column(db.Integer)
    amount_planted = db.Column(db.Integer)
